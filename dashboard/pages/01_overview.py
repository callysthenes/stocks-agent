...
try:
    ...
    with get_sync_db() as db:
        # Channel stats — eagerly load all needed columns
        channels = [
            {"Canal": c.name, "URL": c.url, "Activo": "✅" if c.is_active else "❌",
             "Último Check": c.last_checked_at or "Nunca"}
            for c in db.scalars(select(Channel).where(Channel.is_active == True)).all()  # noqa: E712
        ]

        # Recent videos — eager-load channel to avoid DetachedInstanceError
        recent_videos = [
            {
                "status": str(v.processing_status),
                "title": v.title[:70],
                "url": v.youtube_url,
                "channel": v.channel.name if v.channel else "Unknown",
                "published_at": v.published_at or "N/A",
            }
            for v in db.scalars(
                select(Video)
        ...
        # Status breakdown
        status_counts = [
            (str(row.processing_status), row.count)
    ...
    with col1:
        ...
        if status_counts:
            df_status = pd.DataFrame(status_counts, columns=["Estado", "Vídeos"])
            color_map = {
                "completed": "#2ecc71", "failed": "#e74c3c", "pending": "#95a5a6",
                "transcribing": "#3498db", "embedding": "#9b59b6", "fetching": "#f39c12",
            }
            fig = px.pie(df_status, names="Estado", values="Vídeos",
    ...
    with col2:
        ...
        if top_tickers:
            df_tickers = pd.DataFrame(top_tickers, columns=["Ticker", "Menciones"])
            fig2 = px.bar(df_tickers, x="Ticker", y="Menciones",
